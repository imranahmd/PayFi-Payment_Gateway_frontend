...
// import "./YearlyReport.css"; // Assuming you have some styles for YearlyReport
import Pagination from "../../utils/Pagination";
import generatePDF from "../../utils/GeneratePDF";

const DailyReports = ({ data }) => {
  console.log("dailyrepo",data)
  const pdfRef = useRef();
  ...
  return (
    <div className="p-4 yearly-report">
      <h2 className="text-xl font-bold mb-4">Year Wise with Merchant ID Transaction History</h2>
      ...
      <table id="data-table" className="table-auto border-collapse border border-black my-4">
        ...
        <thead>
          ...
          <th className="border border-black px-4 py-2">merchantId</th>
            ...
            <th className="border border-black px-4 py-2">adminEmailId</th> 
        ...
        <tbody>
          {currentItems.map((transaction, index) => (
            <tr key={index}>
              <td className="border border-black px-4 py-2">{transaction.merchantId}</td>
              <td className="border border-black px-4 py-2">{transaction.transactionId}</td>
              <td className="border border-black px-4 py-2">{transaction.adminEmailId}</td>
              <td className="border border-black px-4 py-2">{transaction.cardHolderName}</td>
